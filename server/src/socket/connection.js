  ...
  io.on('connection', (socket) => {
    ...
    socket.on('workspace:join', async (workspaceId, callback) => {
      try {
        ...
        if (!socket.joinedWorkspaces.has(workspaceId)) {
          const isFirstConnection = await addConnection(workspaceId, socket.user.id);
    ...
    socket.on('disconnect', async () => {
      for (const workspaceId of socket.joinedWorkspaces) {
        try {
          const wasLastConnection = await removeConnection(workspaceId, socket.user.id);
          ...
          console.error('Presence cleanup failed for workspace', workspaceId, err);
        }
      }
    });
    ...
    socket.on('whiteboard:operation', async (workspaceId, objectId, action, data, callback) => {
      try {
        const membership = await getMembership(workspaceId, socket.user.id);
        ...
        callback?.({
          success: false,
          error: err.isOperational ? err.message : 'Failed to apply whiteboard operation. Please try again.',
        });
      }
    });
  });
