...
export async function createInvitation(workspaceId, inviterId, inviterEmail, invitedEmail) {
  ...
  return { ...data, email_sent: emailSent };
}
...
export async function listInvitations(workspaceId, userId) {
  ...
  if (membership.role !== 'owner') {
    throw new AppError('Only the workspace owner can view invitations', 403);
  }
